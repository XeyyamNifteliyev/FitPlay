...
import { shouldEmitMotionCommand, getCooldownMs } from "./gesture-cooldowns";

type MotionInputStatus =
  | "idle"
  | "loading"
  | "camera"
  | "calibrated"
...
type UseMotionInputOptions = {
  sensitivity: PoseSensitivityMode;
  bodyMode: PoseTrackingMode;
...
export function useMotionInput(options: UseMotionInputOptions) {
  const { sensitivity, bodyMode, commands, onMotionEvent } = options;
  ...
  const lastCommandRef = useRef<MotionCommand | null>(null);
  const lastCommandAtRef = useRef(0);
  const lastUiUpdateAtRef = useRef(0);
  const lastFrameAtRef = useRef(0);
  const [trackingMode, setTrackingModeState] =
    useState<PoseTrackingMode>(bodyMode);
  const [sensitivityMode, setSensitivityModeState] =
    useState<PoseSensitivityMode>(sensitivity);

  const [state, setState] = useState<MotionInputState>({
    status: "idle",
  ...
  const start = useCallback(async () => {
    ...
    try {
      ...
      const config = getSensitivityConfig(sensitivityMode);
      ...
      const loop = () => {
        ...
        if (
          command &&
          activeCommands.current.includes(command) &&
  ...
  }, [onMotionEvent, stop, trackingMode, sensitivityMode]);
